...
class Player():
    ...
    def refill_Hunger(self, tankstelle):
        if pygame.key.get_pressed()[pygame.K_SPACE]:
            player_rect = pygame.Rect(self.x, self.y, self.image.get_width(), self.image.get_height())
    ...
    def draw_Hunger(self):
        ...
        text = font.render("Hunger: {}".format(self.hunger -(self.hunger % 1)), True, (0, 0, 0))
...
def draw_game_over(game):
    ...
    text_surface = game.font.render("Game Over", True, (255, 255, 255))
    text_rect = text_surface.get_rect(center=(game.window.get_width() // 2, game.window.get_height() // 2))
...
while(game.running):
    ...
    player.update_Player(mine)
    player.collect_Erz(mine)
    player.deposit_Erz(depot)
    player.refill_Hunger(tankstelle)
    player.draw_Hunger()
    game.update_display()
